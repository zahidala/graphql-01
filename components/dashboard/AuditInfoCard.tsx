...
export const AuditInfoCard = () => {
	...
	const done = formatBytes(totalUp).split(" ")[0];
	const received = formatBytes(totalDown).split(" ")[0];

	const doneUnit = formatBytes(totalUp).split(" ")[1];
	const receivedUnit = formatBytes(totalDown).split(" ")[1];

	const auditRatio = (audit?.auditRatio || 0).toFixed(1);

	const chartData = [
		{ name: `Done (${doneUnit})`, Value: done, fill: "var(--color-done)" },
		{ name: `Received (${receivedUnit})`, Value: received, fill: "var(--color-received)" },
	];
					...
					<CardContent>
						<ChartContainer config={chartConfig}>
							<BarChart data={chartData} layout="vertical">
								...
								<Bar barSize={50} dataKey="Value" layout="vertical" radius={5} />
							</BarChart>
						</ChartContainer>
					</CardContent>
